...
- [x] `/shelf/new`、`/shelf/:id/edit`、`/me` の loader に `requireAuth` を適用
...
- [x] サービス説明・使い方の案内
- [x] ログインボタン / マイページへのリンク
- [x] 新着・人気 Shelf のサンプル表示（任意）
...
- [x] トップページ（フェーズ6）・OGP（フェーズ11）でも `COPY` を使用
...
- [x] `root.tsx` に `AppShell` を導入し、top（`/`）のみナビ非表示にする条件分岐
- [x] スマホ (`< lg`): `<Header>` 上局かつ `<BottomTabs>` 下部固定
- [x] PC (`≥ lg`): `<SideNav>` 左固定 + メインコンテンツは `ml-60` 等でオフセット、`Header` / `BottomTabs` は非表示

### 14-2. スマホヘッダー（`Header.tsx`）

- [x] 左端: my9books ロゴ（`/feed` にリンク）
- [x] 右端: 作者情報アイコン（クリックで `AuthorModal` 表示）
- [x] スマホのみ表示 (`lg:hidden`)
- [x] `AuthorModal.tsx`: 製作者の簡単な説明・GitHub リンク等をモーダル表示
...
- [x] 下部（未ログイン時）: 「ログイン」ボタンのみ表示
- [x] 現在のルートを強調表示（active スタイル）

### 14-5. /search ページ（本棚のみ）

- [x] `app/routes/search.tsx` を新規作成
- [x] テキスト入力で本棚名を検索（`shelves.name LIKE ?`）
- [x] 検索ボックスの下に **例示タグチップ** を表示
  - クリックするとその文字列が検索ボックスに入力され即検索される
  - 固定テキストで表示（定数配列として管理）
  - 例: `私をかたちづくる本` `ライトノベル` `ミステリー` `2025年BEST` `おすすめ`
- [x] 検索結果を本棚カード一覧で表示
- [x] `routes.ts` に `route('search', 'routes/search.tsx')` を追加
...
- [x] `GET /api/shelves` にソートパラメータ追加（`sort=latest|bookmarks|random`）
  - `latest`: `shelves.created_at DESC`
  - `bookmarks`: `shelves.bookmarks_count DESC`
  - `random`: `ORDER BY RANDOM()`（おすすめ代替）
- [x] ページネーション対応（`limit=20&offset=N`）

### 15-2. タイムライン UI

- [x] `/feed` ルートを作成。初期ソートは `latest`
- [x] フルスクリーン縦スクロール型レイアウト（各カードが viewport 1枚）
- [x] ソート切替タブ（新着順 / ブックマーク数順 / おすすめ）を画面上部に配置
- [x] 縦スワイプ（touch & マウスホイール）で前後の本棚に切替
- [x] PC: ↑↓ 矢印キーで操作

### 15-3. GSAP アニメーション

- [x] `gsap` をインストール（`bun add gsap`）
- [x] 縦スワイプ時のカード切替: `gsap.to()` で Y軸スライドアニメーション
- [x] 左右スワイプ時のブックマーク確定アニメーション: ハート or チェックマークが膨らんで消える

### 15-4. 左右スワイプでブックマーク

- [x] 右スワイプ → ブックマーク追加 / 左スワイプ → ブックマーク解除（トグル）
- [x] ログイン済み: `POST/DELETE /api/shelves/:id/bookmarks` を呼び出し
- [x] 未ログイン: スワイプ上限未達の場合はスキップ（ブックマークしない）、上限後はログイン誘導

### 15-5. 未ログインユーザーのスワイプ制限

- [x] `.env` に `FEED_GUEST_SWIPE_LIMIT=30` を追加（フロントで `import.meta.env` 参照）
- [x] `sessionStorage` に `feedGuestSwipeCount` カウンターを保存
- [x] 上限到達時: Clerkログイン画面へ誘導するモーダルを表示し、それ以上スワイプ不可
- [x] Analytics に `feed_guest_limit_reached` イベントを送信（平均離脱スワイプ数の計測用）
...
- `app/routes/home.tsx` — top フッター追加・feed CTA 追加

### 16-0. top → feed への CTA（最優先）

- [x] Hero セクションに「みんなの本棚を見てみる →」ボタンを **最も目立つ一番上の主要アクション** として配置
  - ログイン済み・未ログイン問わず常に表示
  - 既存のログイン・マイページボタンより上に配置し、より大きく目立つスタイル
  - `/feed` へリンク

### 16-1. /me の Shelf 作成 UI 変更

- [x] 右上の「＋新しい本棚を作る」ボタンを削除
- [x] Shelf一覧グリッドの **先頭（左端）** に同サイズの「新規作成カード」を配置
  - 中央に大きな ＋ アイコン
  - 未ログイン不可（そもそも /me は認証必須のため考慮不要）
  - クリックで `/shelf/new` へ遷移
- [x] 一つもShelfがない場合は画面中央に「まだ本棚がありません。最初の9冊を選んでみよう。」のメッセージと新規作成カードを表示
...
- [x] `home.tsx` のみにフッターを追加（他ページには表示しない）
- [x] 表示内容（上から順）:
  1. `© 2026 my9books` — コピーライト
...
**楽天ブックス API ・ Google Books API の利用規約違反に該当するため废止。**

両 API の共通制約:
- 書影は「倍客サービスへの送客を目的とした標準的な表示」のみ許可
- キャッシュ・二次配布・再利用禁止
- アフィリエイトリンクと紐づいた表示が前提

スクリーンショット生成が該当する問題:
1. **書影を PNG に焼き込む = 複製・キャッシュ** — 明確に禁止事項
2. **生成画像を SNS に貧る = 二次配布** — 翼起大
3. **元サイトへのリンクが消える = 送客目的から逸脱** — 規約目的自体を損なう

コードは `git stash` で差し戻し済み。
...
| フェーズ 13: SEO対策            | 🔲 未着手                                 |
| フェーズ 14: 共通レイアウト     | ✅ 完了                                   |
| フェーズ 15: /feed タイムライン | ✅ 完了                                   |
| フェーズ 16: マイページ改善     | 🔲 未着手                                 |
| フェーズ 17: v3 通知機能        | 🔲 未着手                                 |
| フェーズ 18: シェア画像生成     | ❌ 废止（API規約違反のおそれ）         |
