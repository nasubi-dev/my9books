...
- [ ] `shelves` テーブルに insert
- [ ] 作成した shelf を返す

### 3-2. `GET /api/shelves/:id`（Shelf取得）

- [ ] `shelves` + `shelf_books` を JOIN して取得
- [ ] `shelf_books` を position 順にソート
- [ ] `books` から `affiliate_url` を取得してマージ
- [ ] 404ハンドリング

### 3-3. `PATCH /api/shelves/:id`（Shelf名変更）

- [ ] 認証確認 + オーナーチェック
- [ ] `name` を更新、`updated_at` も更新

### 3-4. `DELETE /api/shelves/:id`（Shelf削除）

- [ ] 認証確認 + オーナーチェック
- [ ] CASCADE で `shelf_books` も削除される（schema設定済み）

### 3-5. `POST /api/shelves/:id/view`（閲覧数インクリメント）

- [ ] `view_count` を +1 する（`UPDATE shelves SET view_count = view_count + 1`）

### 3-6. `POST /api/shelves/:id/books`（本を追加）

- [ ] 認証確認 + オーナーチェック
- [ ] 最大9冊チェック（9冊超えは 400）
- [ ] `books` テーブルに isbn + affiliate URL を upsert
- [ ] `shelf_books` に insert

### 3-7. `PATCH /api/shelves/:id/books/:isbn`（感想・ネタバレ・順番更新）

- [ ] 認証確認 + オーナーチェック
- [ ] `review`、`is_spoiler`、`position` を部分更新
...
- [ ] 認証確認 + オーナーチェック
- [ ] `[{ isbn, position }]` 配列を受け取り一括更新

### 3-10. `GET /api/users/:userId/shelves`（ユーザーのShelf一覧）

- [ ] `shelves` を `user_id` でフィルタして返す
...
- [ ] `loader` で `GET /api/shelves/:id` を call
- [ ] `POST /api/shelves/:id/view` でカウントインクリメント
- [ ] 書影取得: isbn をキーに楽天ブックス → Google Books の順で fetch（Skeleton UI で待機）

### 4-2. UI

- [ ] 9冊グリッドレイアウト（Tailwind）
- [ ] Skeleton UI（書影取得中のプレースホルダー）
- [ ] 書影のプログレッシブ表示（取得完了次第差し替え）
- [ ] ネタバレ本はデフォルト隠し表示 + タップで解除
